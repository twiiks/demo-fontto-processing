...
def trim_resize_PIL(image_input, width, height, border):
    ...
    for path, _, images in os.walk(img_dir):
        ...
        for image in images:
            ...
            with Image.open("%s/%s" % (path, image)) as image_input:
                image_output = trim_resize_PIL(image_input, width, height, border)
                ...
                if '/' in path:
                    ...
                    path_out = './'
                full_name = "%s/%s_%dx%d.%s" % (path_out, name, width, height,
                                                ext)
